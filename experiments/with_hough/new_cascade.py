...
def round(version, configsPath, file_list, index):
    pathImage = saveAndLoad(False, configsPath, file_list, index)
    frame = cv.imread(pathImage, 0)
    boxes = version.detectMultiScale(frame, scaleFactor=getScaleFactor(),
                                minNeighbors=getMinNeighbors(),
                                minSize=getMinSize(),
                                maxSize=getMaxSize())
    return pathImage, frame, boxes

def applyHough(img, cimg, boxes, color):
    for (x, y, w, h) in boxes:
        # crop_img = img[y:y+3*h, x:x+w]
        cv.rectangle(cimg, (x, y), (x+w, y+h), GREEN, 2)
        print("x, y, w, h", x, y, w, h)
        crop_img = img[y:y+3*h, x:x+w]
        if len(crop_img) == 0:
            continue 
        circles = cv.HoughCircles(img,cv.HOUGH_GRADIENT,1,3, param1=100,param2=30,minRadius=1,maxRadius=100)
...
if __name__ == "__main__":
    ...
    while True:
        pathImage, frame, boxes = round(v2, configsPath, onlyfiles, index)
       
        _, _, boxes2 = round(v5, configsPath, onlyfiles, index)

       
    
        img = cv.imread(pathImage,0)
        cimg = cv.cvtColor(img,cv.COLOR_GRAY2BGR)
        applyHough(img, cimg, boxes2, RED)

        # circles = cv.HoughCircles(img,cv.HOUGH_GRADIENT,10,20, param1=50,param2=30,minRadius=10,maxRadius=100)
        ...
        # cv.imshow('detected circles',cimg)
        # if getShowV1():
        #     drawCascade(frame, boxes, RED)
        # if getShowV2():
        #     drawCascade(frame, boxes1, BLUE)
        # if getShowV3():
        #     drawCascade(frame, boxes2, GREEN)
