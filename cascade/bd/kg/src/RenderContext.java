...
public class RenderContext extends Bitmap
{
			...
			float dark, int lighter)
	{
		...
		{
			FillTriangle(v1, v2, v3, texture, dark, lighter);
			return;
		}
				...
				ClipPolygonAxis(vertices, auxillaryList, 2))
		{
			...
			{
				FillTriangle(initialVertex, vertices.get(i),
						vertices.get(i + 1), texture, dark, lighter);
			}
		}
	}
			...
			float dark, int lighter)
	{
		...
		Matrix screenSpaceTransform = 
				new Matrix().InitScreenSpaceTransform(GetWidth()/2, GetHeight()/2);
		Matrix identity = new Matrix().InitIdentity();
			...
			float dark, int lighter)
	{
		...
		ScanEdges(gradients, topToBottom, topToMiddle, handedness, texture, dark, lighter);
		ScanEdges(gradients, topToBottom, middleToBottom, handedness, texture, dark, lighter);
	}

	private void ScanEdges(Gradients gradients, Edge a, Edge b,
			boolean handedness, Bitmap texture, float dark, int lighter)
	{
		Edge left = a;
		Edge right = b;
		if(handedness)
		...
		{
			DrawScanLine(gradients, left, right, j, texture, dark, lighter);
			left.Step();
			right.Step();
		}
	}
			...
			float dark, int lighter)
	{
		...
		for(int i = xMin; i < xMax; i++)
		{
			...
			if(depth < m_zBuffer[index])
			{
				...
				CopyPixel(i, j, srcX, srcY, texture, lightAmt, ooo, dark, lighter);
			}
